export const changeLikes = (state, action) => {
  ...
  switch (act) {
    case "add":
      updateMovies = movies.map((movie) =>
        movie.id === id ? { ...movie, likes: movie.likes + 1 } : movie
      );
      updateUpdatedFilms = updatedFilms.map((movie) =>
        movie.id === id ? { ...movie, likes: movie.likes + 1 } : movie
      );
      return {
        ...state,
        movies: updateMovies,
        updatedFilms: updateUpdatedFilms,
      };
    case "remove":
      updateMovies = movies.map((movie) =>
        movie.id === id ? { ...movie, likes: movie.likes - 1 } : movie
...
export const setRatingStars = (state, action) => {
  ...
  const updateMovies = movies.map((movie) =>
    movie.id === id ? { ...movie, stars } : movie
  );
  const updateUpdatedFilms = updatedFilms.map((movie) =>
    movie.id === id ? { ...movie, stars } : movie
  );
...
export const searchByName = (state, action) => {
  ...
  const choosenMovies = allMovies.filter(({ title }) =>
    new RegExp(action.payload, "i").test(title)
  );
...
export const editMovie = (state, action) => {
  ...
  const updateMovies = movies.map((movie) =>
    movie.id === id ? action.payload : movie
  );
  const updateUpdatedFilms = updatedFilms.map((movie) =>
    movie.id === id ? action.payload : movie
  );
...
export const deleteMovie = (state, action) => {
  ...
  const updateUpdatedFilms = updatedFilms.filter((movie) =>
    movie.id === id ? false : true
  );
  return { ...state, movies: updateMovies, updatedFilms: updateUpdatedFilms };
};
